...
class Animation2DMode : public RazzleMode {
  public:
    ...
    virtual bool canRun() { return _m->width() == 2 && _m->height() == 25; }
  private:
...
void Animation2DMode::draw(RazzleMatrix* frame) {
  ...
  if (drawFrame) {
    ...
    uint16_t x = (uint32_t)_Animation2DFrames[_frameIndex].x * frame->width() / 100;
    uint16_t y = (uint32_t)_Animation2DFrames[_frameIndex].y * frame->height() / 100;
    uint16_t w = (uint32_t)_Animation2DFrames[_frameIndex].w * frame->width() / 100;
    uint16_t h = (uint32_t)_Animation2DFrames[_frameIndex].h * frame->height() / 100;
    if (w==0) { w = 1; }
    if (h==0) { h = 1; }

    frame->fillRect(x, y, w, h, _Animation2DFrames[_frameIndex].color);
  }
}
