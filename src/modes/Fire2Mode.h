...
class Fire2Mode : public RazzleMode {
  public:
    ...
    virtual bool canRun() { return _m->numPixels() > 1; }
    virtual void begin() {
      RazzleMode::begin();
      int w = _m->width();
      int h = _m->height();
...
// here we go
void Fire2Mode::draw(RazzleMatrix* frame) {
  ...
  for (uint8_t x = 0; x < Width; x++) {
    // draw
    frame->drawPixelCRGB(x, Height-1, ColorFromPalette( HeatColors_p, _noise[x*Height+0]));
  ...
  for (uint8_t y = 0; y < Height - 1; y++) {
    for (uint8_t x = 0; x < Width; x++) {
      frame->drawPixelCRGB(x, y, ColorFromPalette( HeatColors_p, _heat[x*Height + y]));
    }
  }
}
